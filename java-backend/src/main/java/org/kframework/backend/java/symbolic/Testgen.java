...
public class Testgen {
    ...
    private ConstrainedTerm createInitialTerm(org.kframework.kil.Term cfg) {
        ...
        Term symbolicTerm = TestgenKCellEmptier.emptyK(transformedTerm, TermContext.builder(globalContext).build());
        ConstrainedTerm initialTerm = new ConstrainedTerm(symbolicTerm, TermContext.builder(globalContext).build());
    ...
    private Map<Variable, TestgenDepthLimits> getInitialDepthLimits(
        ...
        PSet<Variable> varSet = initialTerm.variableSet();
    ...
    private static ConstrainedTerm buildPatternResult(
        ...
        /* apply the constraints substitution on the rule LHS (this is the pattern) */
        context.setTopConstraint(constraint);
        Set<Variable> substitutedVars = Sets.union(rule.freshConstants(), rule.matchingVariables());
        constraint = constraint.orientSubstitution(substitutedVars);
        if (rule.isCompiledForFastRewriting()) {
            ...
            term = rule.leftHandSide().substituteAndEvaluate(constraint.substitution(), context);
        }
    ...
    public TestgenResults generate(
        ...
        ConstrainedTerm goalPattern = buildPattern(goalPatternRule, TermContext.builder(globalContext).build());
    ...
    fullSearch:
        ...
        newSearch:
            while (!deque.isEmpty() && states < SEARCH_STATE_LIMIT) {
                ...
                for (Pair<ConjunctiveFormula, Boolean> unificationConstraintpair : currentState.getConstrainedTerm().unify(goalPattern,null,null,null)) {

                    ConjunctiveFormula unificationConstraint = unificationConstraintpair.getKey();
                    ...
                    PSet<Variable> varSet = currentState.getConstrainedTerm().variableSet();
                    for (Variable var : varSet) {
                        ...
                        for (Rule rule : rules) {
                            ...
                            lhs = new ConstrainedTerm(
                                    CellCollection.singleton(CellLabel.K, var, definition.configurationInfo().getCellForConcat(var).get(), definition),
                                    TermContext.builder(globalContext).build());
                            ...
                            ConstrainedTerm rulePattern = new ConstrainedTerm(
                                    CellCollection.singleton(CellLabel.K, kCellContents.get(0),null, definition),
                                    lhs.termContext());

                            for (Pair<ConjunctiveFormula, Boolean> unreachablePair : lhs.unify(rulePattern,null,null,null)) {

                                ConjunctiveFormula unreachableSub = unreachablePair.getKey();
                                rhs = buildResult(rule, unreachableSub, currentState.getTerm(), currentState.getTermContext());
                    ...
                    String query = KILtoSMTLib.translateConstraint(currentState.getConstraint().addAndSimplify(unificationConstraint,currentState.getTermContext()));
                    ...
                    JavaSymbolicObject nextResult = initialTerm.substitute(currentState.substitution()).substitute(z3sub);
                    results++;
                ...
                for (Rule rule : rules) {
                    ...
                    for (Pair<ConjunctiveFormula, Boolean> unificationConstraintpair : currentState.getConstrainedTerm().unify(rulePattern,null,null,null)) {

                        ConjunctiveFormula unificationConstraint = unificationConstraintpair.getKey();
                        ConstrainedTerm result;
                        if(rule.containsAttribute("testgen-pattern")) {
                            result = buildPatternResult(rule, unificationConstraint, currentState.getTerm(), currentState.getTermContext());
                        }
                        else {
                            result = buildResult(rule, unificationConstraint, currentState.getTerm(), currentState.getTermContext());
                        }
