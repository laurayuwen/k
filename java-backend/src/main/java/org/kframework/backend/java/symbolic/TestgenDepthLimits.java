...
public class TestgenDepthLimits {
     ...
     */
    private transient int hashCode = Constants.NO_HASHCODE;
    ...
    public int hashCode() {
        ...
        if (h == Constants.NO_HASHCODE) {
            h = 1;
            h = h * Constants.HASH_PRIME + remainingDepth;
            h = h * Constants.HASH_PRIME + rejectName.hashCode();
            h = h * Constants.HASH_PRIME + sequenceName.hashCode();
            h = h * Constants.HASH_PRIME + sequenceVariables.hashCode();
            h = h * Constants.HASH_PRIME + remainingSequencing;
            h = h * Constants.HASH_PRIME + nestingLimits.hashCode();
            h = h == 0 ? 1 : h;
            hashCode = h;
        }
        return h;
